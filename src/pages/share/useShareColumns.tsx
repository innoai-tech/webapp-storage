import { ILinkShareWithUser, getShareObject, putShareLinkState } from "@src/src-clients/storage";
import { computed, createVNode, onMounted } from "vue";
import { useRequest } from "vue-request";
import { Button, message, Modal } from "ant-design-vue";
import { ExclamationCircleOutlined } from "@ant-design/icons-vue";
import { toFullTime } from "@src/utils/date";
import { useListShareStore } from "@src/pages/share/ShareList";
...
export const useColumns = () => {
  ...
  const { run: sharePutState } = useRequest(putShareLinkState, {
    manual: true,
    onSuccess() {
  ...
  return [
    {
      ...
      dataKey: "path",
      width: 200,
      cellRenderer({ rowData }: { rowData: ILinkShareWithUser }) {
                  ...
                  getShareObject.getConfig(
                    {
                      ...
                      shareID: rowData.shareID,
                      path: rowData.path.endsWith("/") ? undefined : last(rowData.path.split("/")),
                    },
                    false,
                  ).url
